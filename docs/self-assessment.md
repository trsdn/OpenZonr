...
| `L04` | partial | A String Catalog exists per target (`Sources/OpenZonrCore/Resources/Localizable.xcstrings`, `Sources/OpenZonrApp/Resources/Localizable.xcstrings`), and `Scripts/check-localization-keys.py` detects missing and orphaned keys between source and catalog (`--sync` to add missing ones). It currently passes: `OpenZonrCore`'s 141 `L.string(...)` keys are all cataloged, `OpenZonrApp` has none yet. The user-facing surface (`OpenZonrApp`) has not been migrated to the catalog-driven pattern, so this is not yet complete for the product as a whole, and the check is not wired into CI. |
