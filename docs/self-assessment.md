...
| `L01` | pass | The README states the primary language as English (updated with #83); the app's own UI is bilingual (English source, German translation) rather than a single declared UI language. |
| `L02` | pass | Sampled `MenuContent`, `StatusWindow`, `ActivityWindow`, the editor views and the command line help: `OpenZonrApp` and `OpenZonrCore`'s user-facing strings now route through `L.string(...)`/`localized(...)` with an English source value (#83). The command line's own `--help`/diagnostic text in `OpenZonrMac` is not yet migrated (tracked as a follow-up). |
| `L03` | pass | The README's Language and Privacy section states English as the primary/source language and that the UI is bilingual via a String Catalog, linking the tracking issues. |
| `L04` | pass | A String Catalog exists per target (`Sources/OpenZonrCore/Resources/Localizable.xcstrings`, `Sources/OpenZonrApp/Resources/Localizable.xcstrings`), and `Scripts/check-localization-keys.py` detects missing and orphaned keys between source and catalog (`--sync` to add missing ones). Both targets are fully cataloged with zero missing or orphaned keys: `OpenZonrCore` 141 keys, `OpenZonrApp` 254 keys. The check is a documented command, not yet wired into CI (a manual step, per the standard's "validation command or CI check" either/or). |
...
| `L07` | fail | `docs/`'s prose (and several of its file names) are still German, pending its own translation pass (#82 follow-up). README, `AGENTS.md`, code comments and identifiers in `Sources/`/`Tests/` (updated with #82/#83, file by file), the workflows, `.github/` files, and recent commit messages and release notes are English. |
| `X01` | pass | Read from source: standard SwiftUI controls and menu items; the drag gesture for zones has a keyboard-reachable equivalent in the menu ("Pin Frontmost Window Here") and the rule editor. The app was not operated. |
