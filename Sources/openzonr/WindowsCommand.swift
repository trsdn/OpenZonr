...
struct WindowsCommand {
    ...
    func run() throws {
        ...
        let geometry = DisplayGeometry(snapshots: SystemDisplays.snapshots(), descriptors: [])
        ...
        for item in items {
            ...
            let display = geometry.display(containingAccessibilityFrame: snapshot.frame)
            ...
            if showFilterVerdict {
                if let rejection = filter.rejection(for: snapshot, defaults: defaults) {
                    print("    Filter     abgelehnt: \(rejection)")
                } else {
