...
struct OpenZonrCLI {
    ...
    KONFIGURATION
      Standardpfad: ~/.config/openzonr/config.json
      Überschreibbar mit --config. Eine gültige Vorlage liegt unter
      Examples/openzonr.config.json; die echten Bildschirmidentitäten liefert
      "openzonr displays --config-fragment".
        ...
        do {
            ...
            case "watch":
                ...
                let command = WatchCommand(
                    configurationURL: path.map {
                        URL(fileURLWithPath: ($0 as NSString).expandingTildeInPath)
                    } ?? ConfigurationLoader.defaultURL,
            ...
            exit(1)
        } catch let error as ConfigurationLoader.LoadError {
            FileHandle.standardError.write(Data((error.description + "\n").utf8))
            exit(1)
        } catch {
