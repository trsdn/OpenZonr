...
///
/// The comparison is order-independent: the set of attached display identities
/// is translated into the set of aliases the configuration knows, and a profile
/// matches when its fingerprint contains exactly those aliases.
///
/// The translation itself is exact but for one measured exception, which
/// ``DisplayIdentityReconciler`` owns: a display without a serial number whose
/// port index drifted is recognised when — and only when — that is unambiguous
/// on both sides. Identical monitors without a serial number keep needing their
/// port index.
///
...
public struct DefaultProfileResolver: ProfileResolver {
    ...
    ///
    /// The lookup goes through ``DisplayIdentityReconciler``, so a serial-less
    /// display whose `CGDisplayUnitNumber` drifted still finds its descriptor as
    /// long as that is unambiguous. Building the reconciler here rather than
    /// relying on the caller keeps the resolver correct on its own; resolving an
    /// already-reconciled identity is a no-op, so passing a reconciled
    /// fingerprint in costs nothing.
    private func aliases(for fingerprint: SetupFingerprint, in configuration: Configuration) -> Set<DisplayAlias>? {
        ...
        let reconciler = configuration.displayReconciler(observing: Array(fingerprint.displays))

        var aliases: Set<DisplayAlias> = []
        for identity in fingerprint.displays {
            guard let alias = identityToAlias[reconciler.resolve(identity)] else { return nil }
